...
MODULE global
   ...
   ENUM, BIND(C)  
      ENUMERATOR VACUUM, SPECULAR, DIFFUSE, CLL, REACT, AXIS, PERIODIC_MASTER, PERIODIC_SLAVE, EMIT, WB_BC, PISTON
   END ENUM
   ...
   TYPE BOUNDARY_CONDITION_DATA_STRUCTURE
      ...
      INTEGER(KIND(VACUUM)), DIMENSION(100) :: PARTICLE_BC = VACUUM !! DBDBDBD Maximum 100 species
   ...
   ENUM, BIND(C)  
      ENUMERATOR UNIFORM, THERMIONIC, THERMIONIC_ENHANCED, FIELD, EVAPORATION
   END ENUM
   ...
   LOGICAL, DIMENSION(:), ALLOCATABLE :: IS_CONDUCTIVE
   INTEGER, DIMENSION(:), ALLOCATABLE :: CONDUCTOR_NODEMAP
   INTEGER, DIMENSION(:), ALLOCATABLE :: CONDUCTOR_PGMAP
   REAL(KIND=8), DIMENSION(:), ALLOCATABLE :: CONDUCTOR_BIASMAP
   ...
   REAL(KIND=8) :: PARTLOAD_FRACSAMPLE = 1
   ...
   CLASS(VELOCITY_DISTRIBUTION_STRUCTURE), POINTER :: VDF_BOUND
   ...
   TYPE CONNECTED_COND_SURFACE
      ...
      CHARACTER(LEN=256) :: CONDUCTIVE_PART_NAME
      INTEGER            :: CONDUCTIVE_PART_ID
      INTEGER            :: MASTER_NODE = -1

      INTEGER            :: BIAS_CONDUCTOR_PART_ID = -1
      REAL(KIND=8)       :: BIAS_VOLTAGE = 0.d0
      REAL(KIND=8)       :: BIAS_RHS = 0.d0
      
      CHARACTER(LEN=256), DIMENSION(:), ALLOCATABLE :: GROUP_NAMES
      INTEGER, DIMENSION(:), ALLOCATABLE            :: GROUP_ID


      REAL(KIND=8) :: SUM_METAL_CHARGE
      REAL(KIND=8) :: SURFACE_POTENTIAL

   END TYPE CONNECTED_COND_SURFACE

   TYPE(CONNECTED_COND_SURFACE), DIMENSION(:), ALLOCATABLE :: CONNECTED_COND_SURFACES
   ...
   TYPE ELECTRON_FLUID
      ...
      ! In case the electron fluid is confined in part of the domain only
      LOGICAL :: MASKING = .FALSE.
      CHARACTER*64 :: MASKING_CRITERION_PARTICLE_NAME, MASKING_CRITERION_RELATION
      REAL(KIND=8) :: MASKING_CRITERION_DENSITY
      INTEGER, ALLOCATABLE :: MASK(:) ! @ HAS TO BE SET TO ONES DURING INITIALIZATION PHASE
