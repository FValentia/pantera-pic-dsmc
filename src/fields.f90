...
#include <petsc/finclude/petsc.h>
   ...
   SUBROUTINE ASSEMBLE_POISSON
      ...
      IF (.NOT. ALLOCATED(IS_CONDUCTIVE)) ALLOCATE(IS_CONDUCTIVE(0:SIZE-1))
      IS_CONDUCTIVE = .FALSE.
      IF (.NOT. ALLOCATED(CONDUCTOR_NODEMAP)) ALLOCATE(CONDUCTOR_NODEMAP(0:SIZE-1))
      CONDUCTOR_NODEMAP = -1
      IF (.NOT. ALLOCATED(CONDUCTOR_PGMAP)) ALLOCATE(CONDUCTOR_PGMAP(0:SIZE-1))
      CONDUCTOR_PGMAP = -1
      IF (.NOT. ALLOCATED(CONDUCTOR_BIASMAP)) ALLOCATE(CONDUCTOR_BIASMAP(0:SIZE-1))
      ...
      CALL MatSetOption(Amat, MAT_NEW_NONZERO_ALLOCATION_ERR, PETSC_FALSE, ierr)
      ...
      IF (GRID_TYPE == UNSTRUCTURED) THEN
         IF (DIMS == 1) THEN
            ...
            DO I = 1, NCELLS
               ...
               DO J = 1, 2
                  ...
                  IF (EDGE_PG .NE. -1) THEN
                     ...
                     IF (GRID_BC(EDGE_PG)%FIELD_BC == CONDUCTIVE_BC) THEN
                        IF (J==1) THEN
                           IS_CONDUCTIVE(V1-1) = .TRUE.

                           IF (CONNECTED_COND_SURFACES(GRID_BC(EDGE_PG)%CONDUCTIVE_PART_ID)%MASTER_NODE == -1) THEN
                              CONNECTED_COND_SURFACES(GRID_BC(EDGE_PG)%CONDUCTIVE_PART_ID)%MASTER_NODE = V1
                           END IF
                           CONDUCTOR_NODEMAP(V1-1) = VMN-1

                           CONDUCTOR_PGMAP(V1-1) = GRID_BC(EDGE_PG)%CONDUCTIVE_PART_ID
                           CONDUCTOR_BIASMAP(V1-1) = BIAS
                        ELSE IF (J==2) THEN
                           IS_CONDUCTIVE(V2-1) = .TRUE.

                           IF (CONNECTED_COND_SURFACES(GRID_BC(EDGE_PG)%CONDUCTIVE_PART_ID)%MASTER_NODE == -1) THEN
                              CONNECTED_COND_SURFACES(GRID_BC(EDGE_PG)%CONDUCTIVE_PART_ID)%MASTER_NODE = V2
                           END IF
                           CONDUCTOR_NODEMAP(V2-1) = VMN-1

                           CONDUCTOR_BIASMAP(V2-1) = BIAS
                           CONDUCTOR_PGMAP(V2-1) = GRID_BC(EDGE_PG)%CONDUCTIVE_PART_ID
                        END IF
                     END IF
                  END IF
               END DO
            ...
            DO I = 1, NCELLS
               ...
               !!! OLDER ROUTINE, BUT EQUIVALENT, REPLACED BY THE ABOVE FOR BETTER HANDLING !!!
         ...
         ELSE IF (DIMS == 2) THEN
                     ...
                     IF (GRID_BC(EDGE_PG)%FIELD_BC == CONDUCTIVE_BC) THEN
                        IF (J==1) THEN
                           ...
                           IF (CONNECTED_COND_SURFACES(GRID_BC(EDGE_PG)%CONDUCTIVE_PART_ID)%MASTER_NODE == -1) THEN
                              CONNECTED_COND_SURFACES(GRID_BC(EDGE_PG)%CONDUCTIVE_PART_ID)%MASTER_NODE = V1
                           END IF
                           VMN = CONNECTED_COND_SURFACES(GRID_BC(EDGE_PG)%CONDUCTIVE_PART_ID)%MASTER_NODE
                           CONDUCTOR_NODEMAP(V1-1) = VMN-1
                           CONDUCTOR_NODEMAP(V2-1) = VMN-1
                        ...
                        ELSE IF (J==2) THEN
                           ...
                           VMN = CONNECTED_COND_SURFACES(GRID_BC(EDGE_PG)%CONDUCTIVE_PART_ID)%MASTER_NODE
                           CONDUCTOR_NODEMAP(V2-1) = VMN-1
                           CONDUCTOR_NODEMAP(V3-1) = VMN-1

                           CONDUCTOR_PGMAP(V2-1) = GRID_BC(EDGE_PG)%CONDUCTIVE_PART_ID
                           CONDUCTOR_PGMAP(V3-1) = GRID_BC(EDGE_PG)%CONDUCTIVE_PART_ID
                           CONDUCTOR_BIASMAP(V2-1) = BIAS
                           CONDUCTOR_BIASMAP(V3-1) = BIAS
                        ELSE
                           IS_CONDUCTIVE(V1-1) = .TRUE.
                           IS_CONDUCTIVE(V3-1) = .TRUE.

                           IF (CONNECTED_COND_SURFACES(GRID_BC(EDGE_PG)%CONDUCTIVE_PART_ID)%MASTER_NODE == -1) THEN
                              CONNECTED_COND_SURFACES(GRID_BC(EDGE_PG)%CONDUCTIVE_PART_ID)%MASTER_NODE = V3
                           END IF
                           VMN = CONNECTED_COND_SURFACES(GRID_BC(EDGE_PG)%CONDUCTIVE_PART_ID)%MASTER_NODE
                           CONDUCTOR_NODEMAP(V3-1) = VMN-1
                           CONDUCTOR_NODEMAP(V1-1) = VMN-1
            ...
            DO I = 1, NCELLS
               ...
               DO P = 1, 3
                  VP = U2D_GRID%CELL_NODES(P,I) - 1
                  IF (IS_CONDUCTIVE(VP)) VP = CONDUCTOR_NODEMAP(VP)
                  IF (VP >= Istart .AND. VP < Iend) THEN 
                     IF (.NOT. IS_DIRICHLET(VP)) THEN
                        DO Q = 1, 3
                           VQ = U2D_GRID%CELL_NODES(Q,I) - 1
                           IF (IS_CONDUCTIVE(VQ)) VQ = CONDUCTOR_NODEMAP(VQ)

                           KIJ = AREA*(U2D_GRID%BASIS_COEFFS(1,P,I)*U2D_GRID%BASIS_COEFFS(1,Q,I) &
                                     + U2D_GRID%BASIS_COEFFS(2,P,I)*U2D_GRID%BASIS_COEFFS(2,Q,I)) * EPS_REL

                           IF (PIC_TYPE == EXPLICITLIMITED) KIJ = KIJ * (1. + DXLDRATIO(I))
                           IF (AXI) KIJ = KIJ * (Y1+Y2+Y3)/3.
                           CALL MatSetValue(Amat,VP,VQ,KIJ,ADD_VALUES,ierr)
                        END DO
                     END IF
                  END IF
         ...
         ELSE IF (DIMS == 3) THEN
                  ...
                  IF (EDGE_PG .NE. -1) THEN
                     ...
                     IF (GRID_BC(EDGE_PG)%FIELD_BC == CONDUCTIVE_BC) THEN
                        IF (J==1) THEN
                           ...
                           END IF
                           VMN = CONNECTED_COND_SURFACES(GRID_BC(EDGE_PG)%CONDUCTIVE_PART_ID)%MASTER_NODE
                           BIAS = CONNECTED_COND_SURFACES(GRID_BC(EDGE_PG)%CONDUCTIVE_PART_ID)%BIAS_VOLTAGE
                           CONDUCTOR_NODEMAP(V1-1) = VMN-1
                           CONDUCTOR_NODEMAP(V2-1) = VMN-1
                           CONDUCTOR_NODEMAP(V3-1) = VMN-1
                        ...
                        ELSE IF (J==2) THEN
                           ...
                           CONDUCTOR_NODEMAP(V1-1) = VMN-1
                           CONDUCTOR_NODEMAP(V2-1) = VMN-1
                           CONDUCTOR_NODEMAP(V4-1) = VMN-1

                           CONDUCTOR_PGMAP(V1-1) = GRID_BC(EDGE_PG)%CONDUCTIVE_PART_ID
                           CONDUCTOR_PGMAP(V2-1) = GRID_BC(EDGE_PG)%CONDUCTIVE_PART_ID
                           CONDUCTOR_PGMAP(V4-1) = GRID_BC(EDGE_PG)%CONDUCTIVE_PART_ID
                           CONDUCTOR_BIASMAP(V1-1) = BIAS
                           CONDUCTOR_BIASMAP(V2-1) = BIAS
                           CONDUCTOR_BIASMAP(V4-1) = BIAS
                        ELSE IF (J == 3) THEN
                           IS_CONDUCTIVE(V2-1) = .TRUE.
                           IS_CONDUCTIVE(V3-1) = .TRUE.
                           IS_CONDUCTIVE(V4-1) = .TRUE.
                        ...
                        ELSE
                           ...
                           IF (CONNECTED_COND_SURFACES(GRID_BC(EDGE_PG)%CONDUCTIVE_PART_ID)%MASTER_NODE == -1) THEN
                              CONNECTED_COND_SURFACES(GRID_BC(EDGE_PG)%CONDUCTIVE_PART_ID)%MASTER_NODE = V4
                           END IF
                           VMN = CONNECTED_COND_SURFACES(GRID_BC(EDGE_PG)%CONDUCTIVE_PART_ID)%MASTER_NODE
                           BIAS = CONNECTED_COND_SURFACES(GRID_BC(EDGE_PG)%CONDUCTIVE_PART_ID)%BIAS_VOLTAGE
                           CONDUCTOR_NODEMAP(V1-1) = VMN-1
            ...
            IF (BOOL_BIASED_BC) THEN
               DO I = 0, NNODES - 1
                  IF (CONDUCTOR_PGMAP(I) /= -1) THEN
                     IPG = CONNECTED_COND_SURFACES(CONDUCTOR_PGMAP(I))%BIAS_CONDUCTOR_PART_ID

                     IF (IPG /= -1) THEN
                        VMN = CONNECTED_COND_SURFACES(IPG)%MASTER_NODE - 1
                        CONDUCTOR_NODEMAP(I) = VMN
                     END IF
                  END IF
               END DO
            END IF
   ...
   SUBROUTINE APPLY_B_DIPOLE_FIELD(JP, B)
      ...
      REAL(KIND=8), DIMENSION(3) :: PART_POSITION, RELATIVE_POSITION

      PART_POSITION = [particles(JP)%X,particles(JP)%Y,particles(JP)%Z]
      RELATIVE_POSITION = PART_POSITION - DIPOLE_POSITION

      B = MU0/4/PI*(MAGNETIC_MOMENT*RELATIVE_POSITION*DOT(RELATIVE_POSITION, DIPOLE_ORIENTATION)/MAG(RELATIVE_POSITION)**5 &
         - MAGNETIC_MOMENT*DIPOLE_ORIENTATION/MAG(RELATIVE_POSITION)**3)
   ...
   SUBROUTINE SETUP_POISSON_FLUID
      ...
      CALL SNESSetFunction(snes,rvec,FormFunctionFluid,0,ierr)
      ...
      CALL SNESSetJacobian(snes,Jmat,Jmat,FormJacobianFluid,0,ierr)
      CALL SNESSetFromOptions(snes,ierr)
      ...
      IF (DIMS == 1) THEN
         ...
         DO IC = 1, NCELLS
            ...
            IF (U1D_GRID%CELL_PG(IC) == -1) THEN
               EPS_REL = 1.d0
            ELSE
               EPS_REL = GRID_BC(U1D_GRID%CELL_PG(IC))%EPS_REL
            END IF
               ...
               IF (VP >= Istart .AND. VP < Iend) THEN
                  IF (.NOT. IS_DIRICHLET(VP)) THEN
                     DO Q = 1, 2
                        ...
                        VALUETOADD = 0
                        DO IFLUID = 1, N_ELECTRON_FLUIDS
                           N0 = ELECTRON_FLUIDS(IFLUID)%N0
                           T0 = ELECTRON_FLUIDS(IFLUID)%T0
                           PHI0 = ELECTRON_FLUIDS(IFLUID)%PHI0
      ...
      ELSE IF (DIMS == 2) THEN
         DO IC = 1, NCELLS
            ...
            IF (U2D_GRID%CELL_PG(IC) == -1) THEN
               EPS_REL = 1.d0
            ELSE
               EPS_REL = GRID_BC(U2D_GRID%CELL_PG(IC))%EPS_REL
            END IF
                     ...
                     DO Q = 1, 3
                        ...
                        IF (P == Q) THEN
                           ...
                           IF (AXI) THEN
                              ...
                              V1 = U2D_GRID%CELL_NODES(1 + MOD(P-1,3),IC) ! Start from P node in cell I
                              V2 = U2D_GRID%CELL_NODES(1 + MOD(P,3),IC)
                              V3 = U2D_GRID%CELL_NODES(1 + MOD(P+1,3),IC)
                              Y1 = U2D_GRID%NODE_COORDS(2, V1)
                              Y2 = U2D_GRID%NODE_COORDS(2, V2)
                              Y3 = U2D_GRID%NODE_COORDS(2, V3)
                        ...
                        ELSE
                           ...
                           IF (AXI) THEN
                              ...
                              V1 = VP + 1
                              V2 = VQ + 1
                              V3 = U2D_GRID%CELL_NODES(1 + MOD(Q,3),IC)
                              IF (V3 == V1 .OR. V3 == V2) V3 = U2D_GRID%CELL_NODES(1 + MOD(Q+1,3),IC)
                              Y1 = U2D_GRID%NODE_COORDS(2, V1)
                              Y2 = U2D_GRID%NODE_COORDS(2, V2)
                              Y3 = U2D_GRID%NODE_COORDS(2, V3)
      ...
      ELSE IF (DIMS == 3) THEN
         ...
         DO IC = 1, NCELLS
            ...
            IF (U3D_GRID%CELL_PG(IC) == -1) THEN
               EPS_REL = 1.d0
            ELSE
               EPS_REL = GRID_BC(U3D_GRID%CELL_PG(IC))%EPS_REL
            END IF
   ...
   END SUBROUTINE FormFunctionFluid

   SUBROUTINE FormJacobianFluid(snes,x,jac,prec,dummy,ierr_l)
      ...
      IF (MOD(tID, STATS_EVERY) .EQ. 0) CALL ONLYMASTERPRINT1(PROC_ID, 'FormJacobianFluid Called')
                  ...
                  IF (.NOT. IS_DIRICHLET(VP)) THEN
                     DO Q = 1, 2
                        ...
                        DO IFLUID = 1, N_ELECTRON_FLUIDS
                           ...
                           T0 = ELECTRON_FLUIDS(IFLUID)%T0
                           PHI0 = ELECTRON_FLUIDS(IFLUID)%PHI0
                           KQ = QE*QE*N0/(EPS0*EPS_SCALING**2*KB*T0)*LENGTH
                           
                           IF (INT(ELECTRON_FLUIDS(IFLUID)%KAPPA_INDEX) /= 0) THEN
                              KAPPA = ELECTRON_FLUIDS(IFLUID)%KAPPA_INDEX

                              VALUETOADD = VALUETOADD + &
                              KQ *(2.*KAPPA-1.)/(2.*KAPPA-3.) &
                              *(1-QE*(PHI_FIELD_NEW(VQ+1)-PHI0)/(KB*T0*(KAPPA-3./2.)))**(-KAPPA-1./2.)

                           ELSE
                              VALUETOADD = VALUETOADD + &
                              KQ * EXP(QE*(PHI_FIELD_NEW(VQ+1)-PHI0)/(KB*T0))
                           END IF
                        END DO
                  ...
                  IF (.NOT. IS_DIRICHLET(VP)) THEN
                     DO Q = 1, 3
                        ...
                        DO IFLUID = 1, N_ELECTRON_FLUIDS
                           ...
                           T0 = ELECTRON_FLUIDS(IFLUID)%T0
                           PHI0 = ELECTRON_FLUIDS(IFLUID)%PHI0
                           KQ = QE*QE*N0/(EPS0*EPS_SCALING**2*KB*T0)*AREA
                  ...
                  IF (.NOT. IS_DIRICHLET(VP)) THEN
                     DO Q = 1, 4
                        ...
                        DO IFLUID = 1, N_ELECTRON_FLUIDS
                           ...
                           T0 = ELECTRON_FLUIDS(IFLUID)%T0
                           PHI0 = ELECTRON_FLUIDS(IFLUID)%PHI0
                           KQ = QE*QE*N0/(EPS0*EPS_SCALING**2*KB*T0)*VOLUME
   ...
   END SUBROUTINE FormJacobianFluid
   ...
   SUBROUTINE FLUID_ELECTRONS_SURFACE_CHARGING
      ...
      INTEGER :: IC, IP, FACE_PG, IG, IFLUID
      INTEGER :: V1, V2, V3, VV1, VV2, VV3, VMN
      REAL(KIND=8) :: Y1, Y2, AREA, CHARGE, FACTOR
      REAL(KIND=8) :: FLUX1, FLUX2, FLUX3
      REAL(KIND=8) :: N0, T0, PHI0, KAPPA
      REAL(KIND=8) :: E0, V0, B0, AK, DOT_PRODUCT
               ...
               DO IP = 1, 2
                  ...
                  IF ( (GRID_BC(FACE_PG)%FIELD_BC == DIELECTRIC_BC & 
                     ...
                     DO IFLUID = 1, N_ELECTRON_FLUIDS
                        ...
                        CHARGE = -QE*N0/(EPS0*EPS_SCALING**2)*SQRT(KB*T0/(2*PI*ME))*AREA*DT
                     ...
                     IF (GRID_BC(FACE_PG)%FIELD_BC == DIELECTRIC_BC) THEN
                        SURFACE_CHARGE(V1) = SURFACE_CHARGE(V1) + FLUX1

                     ELSE IF (GRID_BC(FACE_PG)%FIELD_BC == CONDUCTIVE_BC) THEN
                        IG = GRID_BC(FACE_PG)%CONDUCTIVE_PART_ID
                        VMN = CONNECTED_COND_SURFACES(IG)%MASTER_NODE
                        SURFACE_CHARGE(VMN) = SURFACE_CHARGE(VMN) + FLUX1
                     
                     ELSE IF (GRID_BC(FACE_PG)%FIELD_BC == SPICE_NODE_BC) THEN
                        AREA = (YMAX-YMIN)*(ZMAX-ZMIN)
                        FLUX1 = FLUX1/DT*(EPS0*EPS_SCALING**2)*AREA
                        GRID_BC(FACE_PG)%SPICE_NODE_CURRENT = GRID_BC(FACE_PG)%SPICE_NODE_CURRENT + FLUX1
                     END IF
                  END IF
               END DO
            END IF
                  ...
                  IF ( (GRID_BC(FACE_PG)%FIELD_BC == DIELECTRIC_BC &
                     ...
                     IF (GRID_BC(FACE_PG)%FIELD_BC == DIELECTRIC_BC) THEN
                        IF (AXI) THEN
                           ...
                           (FLUX1*(3.*Y1+Y2) + FLUX2*(Y1+Y2))/12.
                           SURFACE_CHARGE(V2) = SURFACE_CHARGE(V2) +&
                           (FLUX1*(Y1+Y2) + FLUX2*(Y1+3.*Y2))/12.
                        ELSE
                           SURFACE_CHARGE(V1) = SURFACE_CHARGE(V1) + (FLUX1/3. + FLUX2/6.)
                           SURFACE_CHARGE(V2) = SURFACE_CHARGE(V2) + (FLUX1/6. + FLUX2/3.)
                        END IF
                     ...
                     ELSE IF (GRID_BC(FACE_PG)%FIELD_BC == CONDUCTIVE_BC) THEN
                        ...
                        VMN = CONNECTED_COND_SURFACES(IG)%MASTER_NODE
                        IF (AXI) THEN
                           SURFACE_CHARGE(VMN) = SURFACE_CHARGE(VMN) + (FLUX1*(4*Y1 + 2*Y2)+FLUX2*(2*Y1 + 4*Y2))/12.
                        ELSE
                           SURFACE_CHARGE(VMN) = SURFACE_CHARGE(VMN) + (FLUX1+FLUX2)/2.
                        END IF

                     ELSE IF (GRID_BC(FACE_PG)%FIELD_BC == SPICE_NODE_BC) THEN
                        AREA = (ZMAX-ZMIN)
                        FLUX1 = FLUX1/DT*(EPS0*EPS_SCALING**2)*AREA
                        FLUX2 = FLUX2/DT*(EPS0*EPS_SCALING**2)*AREA
                        IF (AXI) THEN
                           FLUX1 = FLUX1*(Y1+Y2)/2.
                           FLUX2 = FLUX2*(Y1+Y2)/2.
                        END IF
                        GRID_BC(FACE_PG)%SPICE_NODE_CURRENT = GRID_BC(FACE_PG)%SPICE_NODE_CURRENT + (FLUX1+FLUX2)/2.
                     END IF
                  END IF
               END DO
            END IF
               ...
               DO IP=1, 4
                  ...
                  IF ((GRID_BC(FACE_PG)%FIELD_BC == DIELECTRIC_BC &
                     ...
                     IF (GRID_BC(FACE_PG)%FIELD_BC == DIELECTRIC_BC) THEN
                        ...
                        SURFACE_CHARGE(V2) = SURFACE_CHARGE(V2) + (FLUX1/12. + FLUX2/6.  + FLUX3/12.)
                        SURFACE_CHARGE(V3) = SURFACE_CHARGE(V3) + (FLUX1/12. + FLUX2/12. + FLUX3/6.)

                     ELSE IF (GRID_BC(FACE_PG)%FIELD_BC == CONDUCTIVE_BC) THEN
                        IG = GRID_BC(FACE_PG)%CONDUCTIVE_PART_ID
                        VMN = CONNECTED_COND_SURFACES(IG)%MASTER_NODE
                        SURFACE_CHARGE(VMN) = SURFACE_CHARGE(VMN) + (FLUX1+FLUX2+FLUX3)/3.

                     ELSE IF (GRID_BC(FACE_PG)%FIELD_BC == SPICE_NODE_BC) THEN
                        FLUX1 = FLUX1/DT*(EPS0*EPS_SCALING**2)
                        FLUX2 = FLUX2/DT*(EPS0*EPS_SCALING**2)
                        FLUX3 = FLUX3/DT*(EPS0*EPS_SCALING**2)
                        GRID_BC(FACE_PG)%SPICE_NODE_CURRENT = GRID_BC(FACE_PG)%SPICE_NODE_CURRENT + (FLUX1+FLUX2+FLUX3)/3.
                     END IF
                  END IF
               END DO
            END IF
   ...
   SUBROUTINE GET_FLUID_DENSITY

      IMPLICIT NONE

      INTEGER :: IC, IP, IFLUID, V
      REAL(KIND=8) :: N0, T0, PHI0, KAPPA

      IF (ALLOCATED(RHO_FLUID)) DEALLOCATE(RHO_FLUID)
      ALLOCATE(RHO_FLUID(NNODES))
   ...
   SUBROUTINE DEPOSIT_CHARGE(part_adv)
      ...
      REAL(KIND=8) :: K, RHO_Q, CHARGE, BIAS_VOLTAGE
      ...
      CALL MPI_ALLREDUCE(MPI_IN_PLACE, RHS, SIZE, MPI_DOUBLE_PRECISION, MPI_SUM, MPI_COMM_WORLD, ierr)


      IF (BOOL_CONDUCTIVE_BC) THEN
         DO IG=1, N_CONNECTED_COND_SURFACES
            VMN = CONNECTED_COND_SURFACES(IG)%MASTER_NODE - 1
            ! RHS(VMN) = RHS(VMN) + CONNECTED_COND_SURFACES(IG)%SUM_METAL_CHARGE
            RHS(VMN) = RHS(VMN) + CONNECTED_COND_SURFACES(IG)%BIAS_RHS
         END DO
      END IF
   ...
   SUBROUTINE ASSIGN_CONDUCTIVE_PHI
      ...
      DO I=0, NNODES-1
         IF (IS_CONDUCTIVE(I)) THEN 
            IF (CONDUCTOR_NODEMAP(I) /= I) PHI_FIELD(I+1) = PHI_FIELD(CONDUCTOR_NODEMAP(I)+1) + CONDUCTOR_BIASMAP(I)
         END IF
      END DO
